from pgx.connect_five import State as ConnectFiveState


def _make_connect_five_dwg(dwg, state: ConnectFiveState, config):
    ...
    for xy, stone in enumerate(board):
        ...
        # ndarrayのx,yと違うことに注意
        # svgではヨコがx
        # compute row index by dividing by number of columns (BOARD_WIDTH)
        row = xy // BOARD_WIDTH
        stone_y = row * GRID_SIZE + GRID_SIZE / 2
